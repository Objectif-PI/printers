...
class printers_server(models.Model):
    ...
    custom_user = fields.Boolean(string='Custom User', default=False, help="Check this if you want to use OpenERP User Name instead of a specific user")
    ...
    def _update_jobs(self, nothing=None, which='all', first_job_id=-1):
        ...
        for server in ids:
            ...
            for cups_job_id, job_data in jobs_data.items():
                ...
                printer_id = printer_obj.search([('server_id', '=', server.id), ('code', '=', printer_code)])
...
class printers_list(models.Model):
    ...
    def _command(self, print_type, print_data):
        """
...
#             raise osv.except_osv(_('Error'), _('Connection to the CUPS server failed\nCups server : %s (%s:%s)') % (printer.server_id.server, printer.server_id.address, printer.server_id.port))
        ...
        if print_type == 'report':
            ...
            (data, format) = report.render_report(self.env.cr, self.env.uid, print_data['print_ids'], report_data['report_name'], {'model': datas['model']})
...
#             raise osv.except_osv(_('Error'), _('Unknown command type, unable to print !'))
        ...
        jobid = False
        try:
            jobid = connection.printFile(printer.code, filename, self.env.context.get('jobname', 'OpenERP'), options)
        finally:
    ...
    def send_printer(self, report_id, print_ids):
        """
        Sends a report to a printer
        """
        return self._command('report', {'report_id': report_id, 'print_ids': print_ids})
    ...
    def print_test(self):
        ...
        j_name = 'OpenERP Test Page (id: %d)' % printer.id
